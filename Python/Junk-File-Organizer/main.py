...
current_directory = r"C:\Users\ADMIN\Desktop\Attain-U\Junk-File-Organizer\Folder3"
...
#split all file extensions from the dir
    for f in all_files:_, fext = os.path.splitext(f) 
    if fext not in all_fext:all_fext.append(fext)
#create all dirs to organise files
    for ext in all_fext:
        if ext:os.mkdir(os.path.join(path, ext))

    for f in all_files:_, ext = os.path.splitext(f)
